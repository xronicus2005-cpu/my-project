...
const Chat = () => {
  ...
  const dedupeById = (arr) => Array.from(new Map(arr.map(item => [String(item._id), item])).values());
  ...
  useEffect(() => {
    ...
    const exist = conversations.find(c => Array.isArray(c.members) && c.members.includes(id) && c.members.includes(me._id));
    if (exist) { setActive(exist._id); return; }
  ...
  useEffect(() => { if (connection && active) connection.emit("joinConversation", active); }, [connection, active]);
  ...
  useEffect(() => {
    ...
    const handler = (message) => {
      if (!message || !message._id || message.conversationId !== active) return;
      setMessages(prev => prev.find(m => String(m._id) === String(message._id)) ? prev : [...prev, message]);
    };
    connection.on("receiveMessage", handler);
    return () => connection.off("receiveMessage", handler);
  }, [connection, active]);

  const activeUser = conversations.find(c => c._id === active)?.members.find(m => m !== me?._id);
      ...
      <Box sx={{
        ...
        <Box sx={{ flex: 1, overflowY: "auto", p: 1 }}>
          {conversations.length > 0 ? conversations.map(c => (
            <People key={String(c._id)} conversation={c} person={me} active={active === c._id} onClick={() => { setActive(c._id); setShowContacts(false); }} />
